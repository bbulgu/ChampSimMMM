...
for block_size in [8, 32, 128, 512, 2048]:
    for num_cpus in [1, 4, 16, 64]:
         for mshr_l1d_size in [1, 4, 16, 64, 256]:
            for memory_partitioning_method in ["zero", "basic"]:
                for trace_name in ["649.fotonik3d_s-1B.champsimtrace.xz"]:
                    for broadcast_latency in [30, 120, 480]:
                        ...
                        try_execute_exit(f"bin/champsim --warmup_instructions 0 --simulation_instructions {simulation_instructions} {traces_str} >> results/{trace_name_short}_{simulation_instructions}_{num_cpus}cpus_{block_size}block_{mshr_l1d_size}mshr_{memory_partitioning_method}partitioning_{broadcast_latency}latency.txt")
