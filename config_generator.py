...
for block_size in [512, 2048]:
    for num_cpus in [4, 16]:
         for mshr_l1d_size in [16]:
            for memory_partitioning_method in ["zero", "basic", "shift", "shift6"]:
                for trace_name in ["649.fotonik3d_s-1B.champsimtrace.xz"]:
                    for broadcast_latency in [0]:
                        trace_name_short = trace_name[0:3]
                        file_name = f"results/{trace_name_short}_{simulation_instructions}_{num_cpus}cpus_{block_size}block_{mshr_l1d_size}mshr_{memory_partitioning_method}partitioning_{broadcast_latency}latency.txt"
                        ...
                        try_execute_exit(f"bin/champsim --warmup_instructions 0 --simulation_instructions {simulation_instructions} {traces_str} >> {file_name}")
